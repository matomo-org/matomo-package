...
.PHONY		: checkfetch fixperms checkversions release checkenv builddeb checkdeb checkdebdetail newrelease newversion changelog history clean upload fixsettings
...
cleanup:
		...
		@find matomo/ -type f -name ".eslintrc" -exec rm -f {} \;
		@find matomo/vendor -mindepth 2 -type f -name "autoload.php" -exec rm -f {} \;
...
checkmatomoinstall:
	...
	@for F in $(shell cat debian/matomo.install|grep ^matomo|cut -f1) ; do \
...
checkmatomorootdir:
	@echo -n " [CONF] Checking other files/dir... "
	@for F in $(shell ls matomo) ; do \
		echo -n "  * checking in root dir: $$F"; \
		if [ ! $(shell grep "^matomo/$$F" debian/matomo.install) ]; then \
			echo " $(RED)missing$(NC)."; \
			echo "1" >&2; \
		else \
			echo " $(GREEN)ok$(NC)."; \
